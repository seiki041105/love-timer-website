...
const WorkSchedulePage = ({
  ...
  const [schedules, setSchedules] = useState<{ [key: string]: { seiki: string; ryoma: string } }>({})
  const [selectedDate, setSelectedDate] = useState<string | null>(null)
  const [editingSchedule, setEditingSchedule] = useState<{ date: string; person: "seiki" | "ryoma" } | null>(null)
  ...
  // 金额相关状态
  const [monthlyAmounts, setMonthlyAmounts] = useState<{ seiki: number; ryoma: number }>({ seiki: 0, ryoma: 0 })
  const [editingAmount, setEditingAmount] = useState<{ person: "seiki" | "ryoma"; amount: string } | null>(null)
  ...
  const translations = {
    zh: {
      ...
      visitCount: "访问次数",
                         seikiSchedule: "Seiki的工作计划",
                   ryomaSchedule: "Ryoma的工作计划",
                   noSchedule: "暂无计划",
                   loading: "加载中...",
                   error: "加载失败",
                   monthlyAmount: "本月金额",
    ...
    ja: {
      ...
      reset: "リセット",
      stateSaved: "状態保存済み",
      scheduleSaved: "スケジュール保存済み",
      scheduleDeleted: "スケジュール削除済み",
      visitCount: "訪問回数",
      seikiSchedule: "Seikiのワークスケジュール",
      ryomaSchedule: "Ryomaのワークスケジュール",
      noSchedule: "スケジュールなし",
      loading: "読み込み中...",
      error: "読み込み失敗",
      monthlyAmount: "今月の金額",
    ...
    en: {
      ...
      visitCount: "Visit count",
      seikiSchedule: "Seiki's Work Schedule",
      ryomaSchedule: "Ryoma's Work Schedule",
      noSchedule: "No schedule",
      loading: "Loading...",
      error: "Load failed",
      monthlyAmount: "Monthly Amount",
      seikiAmount: "Seiki Amount",
      ryomaAmount: "Ryoma Amount",
  ...
  const loadSchedulesFromSupabase = async () => {
    try {
      ...
      if (error) {
        ...
        throw new Error(`Supabase错误: ${error.message || '未知错误'}`)
      }
      
      console.log('从Supabase获取到的数据:', data)
      
      // 将数据转换为原来的格式
      const schedulesData: { [key: string]: { seiki: string; ryoma: string } } = {}
      data?.forEach((item: any) => {
        const dateStr = item.date
        if (!schedulesData[dateStr]) {
          schedulesData[dateStr] = { seiki: '', ryoma: '' }
        }
        schedulesData[dateStr][item.person as "seiki" | "ryoma"] = item.schedule
      })
      
      console.log('转换后的工作计划数据:', schedulesData)
      setSchedules(schedulesData)
    } catch (err: any) {
      console.error('加载工作计划失败:', err)
      setError(err.message || '加载工作计划失败')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  // 保存工作计划到Supabase
  const saveScheduleToSupabase = async (date: string, person: "seiki" | "ryoma", schedule: string) => {
    ...
    try {
      ...
      if (checkError) {
        console.error('检查现有计划时出错:', checkError)
        throw checkError
      }
      ...
      // 同时保存到本地存储
      storage.set("work-schedules", newSchedules)
      ...
      console.error('Supabase保存失败，使用本地存储:', err)
      // 如果Supabase失败，使用本地存储
      const newSchedules = {
        ...schedules,
        [date]: {
  ...
  // 删除工作计划
  const deleteScheduleFromSupabase = async (date: string, person: "seiki" | "ryoma") => {
    try {
      setIsSaving(true)
      setError(null)
      ...
      console.error('Supabase删除失败，使用本地存储:', err)
      // 如果Supabase失败，使用本地存储
      const newSchedules = {
        ...schedules,
        [date]: {
  ...
  // 开始编辑
  const startEditing = (date: string, person: "seiki" | "ryoma") => {
    setEditingSchedule({ date, person })
    setTempSchedule(schedules[date]?.[person] || "")
  }
  ...
  useEffect(() => {
    // 尝试从Supabase加载，如果失败则使用本地存储
    const loadSchedules = async () => {
      try {
        await loadSchedulesFromSupabase()
      } catch (err) {
        console.log('Supabase加载失败，使用本地存储:', err)
        // 从本地存储加载
        const savedSchedules = storage.get("work-schedules", {})
    ...
    const loadAmounts = async () => {
      try {
        await loadMonthlyAmounts()
      } catch (err) {
        console.log('加载金额失败，使用本地存储:', err)
        const savedAmounts = storage.get("monthly-amounts", { seiki: 0, ryoma: 0 })
        setMonthlyAmounts(savedAmounts)
      }
    }
    ...
    if (!visitedPages.includes("workSchedule")) {
      const newVisited = [...visitedPages, "workSchedule"]
      setVisitedPages(newVisited)
      storage.set(STORAGE_KEYS.VISITED_PAGES, newVisited)
    }
  }, [visitedPages, setVisitedPages, currentMonth])

  const handleSaveSchedule = async () => {
    if (!editingSchedule || !tempSchedule.trim()) return
    await saveScheduleToSupabase(editingSchedule.date, editingSchedule.person, tempSchedule)
  }

  const handleDeleteSchedule = async () => {
    if (!editingSchedule) return
    await deleteScheduleFromSupabase(editingSchedule.date, editingSchedule.person)
  }

  // 加载月度金额
  const loadMonthlyAmounts = async () => {
    try {
      ...
      const amounts = { seiki: 0, ryoma: 0 }
      data?.forEach((item: any) => {
        amounts[item.person as "seiki" | "ryoma"] = item.amount || 0
      })
      
      setMonthlyAmounts(amounts)
    } catch (err) {
      console.error('加载月度金额失败:', err)
      const savedAmounts = storage.get("monthly-amounts", { seiki: 0, ryoma: 0 })
      setMonthlyAmounts(savedAmounts)
    }
  }
  ...
  // 保存月度金额
  const saveMonthlyAmount = async (person: "seiki" | "ryoma", amount: number) => {
    try {
      const currentYear = currentMonth.getFullYear()
      const currentMonthNum = currentMonth.getMonth() + 1
      ...
      if (checkError) {
        console.error('检查现有金额时出错:', checkError)
        throw checkError
      }
      ...
      // 同时保存到本地存储
      const newAmounts = { ...monthlyAmounts, [person]: amount }
      storage.set("monthly-amounts", newAmounts)
      ...
      console.error('保存金额失败，使用本地存储:', err)
      // 如果Supabase失败，使用本地存储
      setMonthlyAmounts(prev => ({
        ...prev,
        [person]: amount
  ...
  const resetAllSchedules = () => {
    ...
    setEditingSchedule(null)
    setTempSchedule("")
    storage.remove("work-schedules")
    storage.remove("monthly-amounts")
    setMonthlyAmounts({ seiki: 0, ryoma: 0 })
    setShowSaveNotification(true)
    setTimeout(() => setShowSaveNotification(false), 2000)
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 flex flex-col items-center justify-center p-4 relative overflow-hidden">
      ...
      <FloatingElement delay={0.2}>
        <div className="bg-white rounded-3xl shadow-2xl p-8 w-[1200px] max-w-full relative z-10 backdrop-blur-sm bg-opacity-95">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center">
              <BounceButton onClick={onBack} className="mr-4 p-2 rounded-full hover:bg-gray-100">
                <ArrowLeft className="w-6 h-6" />
              </BounceButton>
              <h1 className="text-3xl font-bold text-gray-800">{t.workSchedule}</h1>
            </div>
            <BounceButton
          ...
          <div className="grid grid-cols-7 gap-1 mb-6 w-full">
            {dayNames.map((day, index) => (
              <div key={index} className="p-2 text-center font-semibold text-gray-600 text-sm">
                {day}
              </div>
            ))}
            {getCalendarDays().map((date, index) => {
              const dateStr = formatDate(date)
              const hasSchedule = schedules[dateStr] && (schedules[dateStr].seiki || schedules[dateStr].ryoma)
              
              return (
                <div
                  key={index}
                  className={`p-2 min-h-[80px] border border-gray-200 rounded-lg cursor-pointer transition-all duration-200 ${
                    isToday(date) ? 'bg-blue-100 border-blue-300' : ''
                  } ${
                    !isCurrentMonth(date) ? 'text-gray-400' : 'text-gray-800'
                  } ${
                    hasSchedule ? 'bg-gradient-to-br from-green-50 to-emerald-50 border-green-300' : ''
                  } ${
                    selectedDate === dateStr ? 'bg-gradient-to-br from-purple-100 to-pink-100 border-purple-400 ring-2 ring-purple-300' : ''
                  } hover:bg-gray-50`}
                ...
                >
                  ...
                  {hasSchedule && (
                    <div className="text-xs text-green-600">
                      {schedules[dateStr].seiki && '👩‍💼'}
                      {schedules[dateStr].ryoma && '👨‍💼'}
                    </div>
                  )}
                </div>
              )
                    ...
                    {/* 选中日期的计划编辑 */}
          <div className={`bg-gradient-to-r from-purple-50 to-pink-50 rounded-2xl p-6 shadow-lg transition-all duration-300 w-full ${
            selectedDate ? 'min-h-[400px] opacity-100' : 'min-h-0 opacity-0 pointer-events-none'
          }`}>
            {selectedDate && (
              <>
                <h3 className="text-xl font-bold text-gray-800 mb-4">
                  {formatDateForDisplay(selectedDate)}的工作计划
                </h3>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {/* Seiki的计划 */}
                  <div className="bg-white rounded-xl p-4 shadow-md">
                    <h4 className="font-semibold text-gray-800 mb-2">👩‍💼 Seiki</h4>
                    {editingSchedule?.date === selectedDate && editingSchedule?.person === "seiki" ? (
                      <div>
                        <textarea
                          value={tempSchedule}
                          onChange={(e) => setTempSchedule(e.target.value)}
                          placeholder="添加Seiki的工作计划..."
                          className="w-full h-24 p-3 border border-gray-300 rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-pink-500"
                        />
                      ...
                      <div>
                        <p className="text-gray-700 min-h-[6rem]">
                          {schedules[selectedDate]?.seiki || t.noSchedule}
                        </p>
                        <BounceButton
                          onClick={() => startEditing(selectedDate, "seiki")}
                          className="mt-2 bg-pink-500 text-white px-3 py-1 rounded text-sm hover:bg-pink-600"
                        >
                          {schedules[selectedDate]?.seiki ? '编辑' : '添加'}
                        </BounceButton>
                      </div>
                    )}
                  </div>
                  ...
                  {/* Ryoma的计划 */}
                  <div className="bg-white rounded-xl p-4 shadow-md">
                    <h4 className="font-semibold text-gray-800 mb-2">👨‍💼 Ryoma</h4>
                    {editingSchedule?.date === selectedDate && editingSchedule?.person === "ryoma" ? (
                      <div>
                        <textarea
                          value={tempSchedule}
                          onChange={(e) => setTempSchedule(e.target.value)}
                          placeholder="添加Ryoma的工作计划..."
                      ...
                      <div>
                        <p className="text-gray-700 min-h-[6rem]">
                          {schedules[selectedDate]?.ryoma || t.noSchedule}
                        </p>
                        <BounceButton
                          onClick={() => startEditing(selectedDate, "ryoma")}
                          className="mt-2 bg-blue-500 text-white px-3 py-1 rounded text-sm hover:bg-blue-600"
                        >
                          {schedules[selectedDate]?.ryoma ? '编辑' : '添加'}
                        </BounceButton>
                      </div>
                    )}
                  </div>
                      ...
                      {/* 本月金额部分 */}
            <div className="mt-6 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl p-6 shadow-lg">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Seiki的金额 */}
                <div className="bg-white rounded-xl p-4 shadow-md">
                  <h4 className="font-semibold text-gray-800 mb-2">👩‍💼 {t.seikiAmount}</h4>
                  {editingAmount?.person === "seiki" ? (
                    <div>
                      <input
                        type="number"
                        value={editingAmount.amount}
                        onChange={(e) => setEditingAmount({ person: "seiki", amount: e.target.value })}
                        placeholder="输入金额..."
                      ...
                      <div className="flex gap-2 mt-2">
                        <BounceButton
                          onClick={() => saveMonthlyAmount("seiki", parseFloat(editingAmount.amount) || 0)}
                    ...
                    <div>
                      <p className="text-2xl font-bold text-green-600 mb-2">
                        ¥{monthlyAmounts.seiki.toLocaleString()}
                      </p>
                      <BounceButton
                        onClick={() => setEditingAmount({ person: "seiki", amount: monthlyAmounts.seiki.toString() })}
                ...
                {/* Ryoma的金额 */}
                <div className="bg-white rounded-xl p-4 shadow-md">
                  <h4 className="font-semibold text-gray-800 mb-2">👨‍💼 {t.ryomaAmount}</h4>
                  {editingAmount?.person === "ryoma" ? (
                    <div>
                      <input
                        type="number"
                        value={editingAmount.amount}
                        onChange={(e) => setEditingAmount({ person: "ryoma", amount: e.target.value })}
                        placeholder="输入金额..."
                      ...
                      <div className="flex gap-2 mt-2">
                        <BounceButton
                          onClick={() => saveMonthlyAmount("ryoma", parseFloat(editingAmount.amount) || 0)}
                          className="bg-green-500 text-white px-3 py-1 rounded text-sm hover:bg-green-600"
                        >
                          {t.saveAmount}
                        </BounceButton>
                    ...
                    <div>
                      <p className="text-2xl font-bold text-blue-600 mb-2">
                        ¥{monthlyAmounts.ryoma.toLocaleString()}
                      </p>
                      <BounceButton
                        onClick={() => setEditingAmount({ person: "ryoma", amount: monthlyAmounts.ryoma.toString() })}
            ...
            <div className="mt-4 text-center text-sm text-gray-500">
              {t.visitCount}: {visitedPages.filter((page) => page === "workSchedule").length}
            </div>
        </div>
      </FloatingElement>
    </div>
...
export default function LoveTimerApp() {
  ...
  const [currentPage, setCurrentPage] = useState<
    "home" | "map" | "todo" | "workSchedule" | "cooking" | "gacha" | "calendar" | "anime" | "quiz"
  >("home")
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-2xl w-full relative z-10 backdrop-blur-sm bg-opacity-95">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            ...
            <FloatingElement delay={1.9}>
              <BounceButton
                onClick={() => setCurrentPage("workSchedule")}
              ...
              >
                <div className="text-2xl mb-2">📋</div>
                <span className="text-sm font-semibold text-center leading-tight">{t.workSchedule}</span>
                {visitedPages.includes("workSchedule") && (
                  <div className="absolute -top-1 -right-1 w-3 h-3 bg-green-400 rounded-full animate-pulse"></div>
                )}
              </BounceButton>
            </FloatingElement>
